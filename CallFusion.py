...
def getResult(instanceURI, uname, pw, sql_query):
    instance = instanceURI
    username = uname
    password = pw
    initFusion(instance, username, password)

    trial = 0
    while True:
        try:
            s = io.StringIO(runSQL(instance, sql_query, username, password))
            if s.getvalue().__len__() == 32:
                df_result = pd.DataFrame()
                break
            else:
                df_result = pd.read_xml(s)
                break
        except:
            s.close()
            trial+=1
